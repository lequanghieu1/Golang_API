...
func GetAllModels(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	collection := db.ConnectModelDevices()
...
func CreateModelDevices(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	...
	collection := db.ConnectModelDevices()
	var result bson.M
	errFindUsername := collection.FindOne(context.TODO(), bson.M{"name": name}).Decode(&result)

	if errFindUsername == nil {
		res.JSON(w, 409, "Model does exists")
		return
	}
	...
	if errs != nil {
		res.JSON(w, 500, "Create model device has failed")
		return
	}
...
func EditModelDevice(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	fmt.Println(errFind)
	if errFind != nil {
		res.JSON(w, 404, "Model device Not Found")
		return
	}
...
func DeleteModelDevice(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	id := ps.ByName("id")
	username, err := jwt.ExtractUsernameFromToken(r)
	collection := db.ConnectModelDevices()
